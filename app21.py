...
with col_insights:
    ...
    if queue_on_arrival >= 3:
        insights.append("🔴 **High queue on arrival** — patient joining a congested queue significantly increases breach risk")
    if triage_level == 2:
        insights.append("🔴 **Triage level 2 (Urgent)** — highest-risk triage group in this model; these patients often wait behind level-1 critical cases")
    if needs_lab:
        insights.append("🟡 **Lab test required** — adds 20–90 min downstream; breach risk increases significantly")
    if arrival_hour >= 18:
        insights.append("🟡 **Evening arrival** — ED congestion typically peaks in evening hours")
    if raw_input["num_doctors"] / raw_input["arrival_rate"] < 0.25:
        insights.append("🔴 **Critically understaffed** — doctor-to-arrival ratio below 0.25; breach risk is elevated for all patients")
    if triage_level >= 4 and queue_on_arrival == 0:
        insights.append("🟢 **Low-acuity patient with no queue** — favourable conditions for timely discharge")
    if not insights:
        insights.append("🟢 **No major risk flags detected** — current configuration looks manageable")
    ...
    with tab1:
        ...
        ax1.axvline(num_doctors, color="#333333", linewidth=1.8,
                    linestyle="--", label=f"Current: {num_doctors} doctors", alpha=0.85)
